...
int aml_input_open(struct audio_stream_in *stream, aml_stream_config_t * stream_config, audio_devices_t in_device) {
    int ret = -1;
	...
	handle->device = input_device;

	
    ret = aml_input_function.input_open(&handle->device_handle, stream_config, in_device);
